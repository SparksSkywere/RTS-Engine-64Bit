...
int	RegistryClass::Get_Int( const char * name, int def_value )
{
	...
	if (( ::RegQueryValueEx( Key, name, NULL, &type, (LPBYTE)&data, &data_len ) ==
...
int RegistryClass::Get_Bin_Size( const char * name )
{
	...
	::RegQueryValueEx( Key, name, NULL, NULL, NULL, &size );
	return size;
}
...
void RegistryClass::Get_Bin( const char * name, void *buffer, int buffer_size )
{
	...
	::RegQueryValueEx( Key, name, NULL, NULL, (LPBYTE)buffer, &size );
	return ;
}
...
void	RegistryClass::Get_String( const char * name, StringClass &string, const char *default_string )
{
	...
	DWORD data_size = 0;
	DWORD type = 0;
	LONG result = ::RegQueryValueEx (Key, name, NULL, &type, NULL, &data_size);
	if (result == ERROR_SUCCESS && type == REG_SZ) {

		//
		//	Read the entry from the registry
		//
		::RegQueryValueEx (Key, name, NULL, &type,
   ...
   const char * default_string )
{
	assert( IsValid );
	DWORD type = 0;
	if (( ::RegQueryValueEx( Key, name, NULL, &type, (LPBYTE)value, (DWORD*)&value_size ) ==
...
void	RegistryClass::Get_String( const WCHAR * name, WideStringClass &string, const WCHAR *default_string )
{
	...
	DWORD data_size = 0;
	DWORD type = 0;
	LONG result = ::RegQueryValueExW (Key, name, NULL, &type, NULL, &data_size);
	if (result == ERROR_SUCCESS && type == REG_SZ) {

		//
		//	Read the entry from the registry
		//
		::RegQueryValueExW (Key, name, NULL, &type,
