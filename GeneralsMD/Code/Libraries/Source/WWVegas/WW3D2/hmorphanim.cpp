		 ...
		 (string_list != NULL))
	{
		...
		if (count > 0) {
				  ...
				  entry = ::strstr (entry, delimiter))
			{
				...
				const char *delim_start = ::strstr((const char *)entry_string, delimiter);				
				if (delim_start != NULL) {
					entry_string[int(delim_start - (const char *)entry_string)] = 0;
				}
...
void HMorphAnimClass::Set_Name(const char * name)
{
	...
	if (separator != NULL) {
		...
		int hierarchy_len = int(separator - name);
		::strncpy(HierarchyName, name, hierarchy_len);
		HierarchyName[hierarchy_len] = 0;
	}
