...
//-----------------------------------------------------------------------------
// AnimWindow PUBLIC FUNCTIONS /////////////////////////////////////////////
//-----------------------------------------------------------------------------
AnimWindow::AnimWindow( void )
{
	m_delay = 0;
...
void AnimWindow::setAnimData( 	ICoord2D startPos, ICoord2D endPos, 
...
void AnimateWindowManager::update( void )
{
	...
	if(m_needsUpdate)
	{
		...
		{
			AnimWindow *animWin = *it;
			if (!animWin)
			{
				DEBUG_CRASH(("There's No AnimWindow in the AnimWindow List"));
				return;
			}
...
void AnimateWindowManager::registerGameWindow(GameWindow *win, AnimTypes animType, Bool needsToFinish, UnsignedInt ms, UnsignedInt delayMs)
{
	...
	// Create a new AnimWindow class and fill in it's data.
	AnimWindow *animWin = newInstance(AnimWindow);	
