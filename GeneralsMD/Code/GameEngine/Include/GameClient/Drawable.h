...
public:
  ...
  void onLevelStart();                                                ///< run from GameLogic::startNewGame
	...
	Bool testTintStatus( TintStatus statusBits ) const { return (m_tintStatus & statusBits) != 0; };
  ...
  void imitateStealthLook( Drawable& otherDraw );
	...
	Bool getShadowsEnabled() const { return (m_status & DRAWABLE_STATUS_SHADOWS) != 0; }
  ...
  //	inline UnsignedByte getFullyObscuredByShroudWithCheatSpy(void) {return (UnsignedByte)m_drawableFullyObscuredByShroud | 128;}//8 looks like a zero in most fonts
	...
	Bool getDrawsInMirror() const { return ((m_status & DRAWABLE_STATUS_DRAWS_IN_MIRROR) != 0) || isKindOf(KINDOF_CAN_CAST_REFLECTIONS); }
