...
int W3DTreeBuffer::W3DTreeTextureClass::update(W3DTreeBuffer *buffer)
{
	...
	DX9_ErrorCode(Peek_D3D_Texture()->GetSurfaceLevel(0, &surface_level));
	DX9_ErrorCode(surface_level->GetDesc(&surface_desc));

	DX9_ErrorCode(surface_level->LockRect(&locked_rect, NULL, 0));
...
//	Int numRows = surface_desc.Height/(tilePixelExtent+TILE_OFFSET);
	...
	DX9_ErrorCode(surface_level->UnlockRect());
	surface_level->Release();
	DX9_ErrorCode(D3DXFilterTexture(Peek_D3D_Texture(), NULL, (UINT)0, D3DX_FILTER_BOX));	
	if (TheWritableGlobalData->m_textureReductionFactor) {
		DX9_ErrorCode(Peek_D3D_Texture()->SetLOD((DWORD)TheWritableGlobalData->m_textureReductionFactor));
	}
...
{
	if (Peek_D3D_Texture()) {
		DX9_ErrorCode(Peek_D3D_Texture()->SetLOD((DWORD)LOD));
	}
}
//=============================================================================
...
void W3DTreeBuffer::freeTreeBuffers(void)
{
	...
	if (m_dwTreePixelShader)
		D3D9_DeletePixelShader(DX8Wrapper::_Get_D3D_Device9(), m_dwTreePixelShader);
	m_dwTreePixelShader = 0;

	if (m_dwTreeVertexShader)
		D3D9_DeleteVertexShader(DX8Wrapper::_Get_D3D_Device9(), m_dwTreeVertexShader);
	m_dwTreeVertexShader = 0;
}
...
//#define DEBUG_TEXTURE 1
	...
	if (m_dwTreeVertexShader) {
		...
		DX8Wrapper::_Get_D3D_Device9()->SetVertexShaderConstant(  4, &mat,  4 );
		Vector4 noSway(0,0,0,0);
		DX8Wrapper::_Get_D3D_Device9()->SetVertexShaderConstant(  8, &noSway,  1 );

		// c8 - c8+MAX_SWAY_TYPES - the sway amount.
		for	(i=0; i<MAX_SWAY_TYPES; i++) {
			Vector4 sway4(swayFactor[i].X, swayFactor[i].Y, swayFactor[i].Z, 0);
			DX8Wrapper::_Get_D3D_Device9()->SetVertexShaderConstant(  9+i, &sway4,  1 );
		}
		...
		if ((shroud=TheTerrainRenderObject->getShroud()) != 0) {
			...
			DX8Wrapper::_Get_D3D_Device9()->SetVertexShaderConstant(  32, &offset,  1 );
			width = 1.0f/(width*shroud->getTextureWidth());
			height = 1.0f/(height*shroud->getTextureHeight());
			offset.Set(width, height, 1, 1);
			DX8Wrapper::_Get_D3D_Device9()->SetVertexShaderConstant(  33, &offset,  1 );
		...
		if(TheGlobalData && TheGlobalData->m_useOverbright)
			mulTwoX = 1.0f;	
		DX8Wrapper::_Get_D3D_Device9()->SetPixelShaderConstant(1, D3DXVECTOR4(mulTwoX, mulTwoX, mulTwoX, mulTwoX), 1);
#endif
	...
	for (bNdx=0;bNdx<MAX_BUFFERS; bNdx++) {
		...
		DX8Wrapper::Apply_Render_State_Changes();
		if (m_dwTreeVertexShader) {
			D3D9_SetVertexShader(DX8Wrapper::_Get_D3D_Device9(), m_dwTreeVertexShader);
			DX8Wrapper::_Get_D3D_Device9()->SetTextureStageState(0,  D3DTSS_TEXCOORDINDEX, 0);
			DX8Wrapper::_Get_D3D_Device9()->SetTextureStageState(1,  D3DTSS_TEXCOORDINDEX, 1);
			DX8Wrapper::_Get_D3D_Device9()->SetTextureStageState(1,  D3DTSS_TEXTURETRANSFORMFLAGS, D3DTTFF_DISABLE);
		}
